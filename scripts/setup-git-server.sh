...
echo "=== Done ==="
echo ""
echo "!! SECURITY — a push to this repo runs the pushed code (RCE as you):"
echo "   - Use key-only SSH (disable password auth in sshd_config)."
echo "   - Bind sshd to your Tailscale IP; never expose the SSH/git port publicly."
echo "   - Restrict logins (AllowUsers) to your account only."
echo "   - Optional: export PALMTOP_REQUIRE_SIGNED=1 for the hook to require"
echo "     signed commits (set up git allowed-signers first)."
echo ""

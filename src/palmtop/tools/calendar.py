...
class GoogleCalendarTool(Tool):
    ...
    async def init(self) -> None:
        ...
        if not self._tokens_path.exists():
            log.warning("Google tokens not found — run: python -m palmtop.tools.google_auth")
            return
    ...
    async def _api(self, method: str, path: str, **kwargs) -> httpx.Response:
        ...
        if not self._access_token:
            raise RuntimeError("Not authenticated — run: python -m palmtop.tools.google_auth")
    ...
    async def run(self, query: str) -> str:
        if not self._access_token and not self._tokens_path.exists():
            return "Calendar not connected. Run: python -m palmtop.tools.google_auth"
